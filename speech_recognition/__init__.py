...
class AudioFile(AudioSource):
    ...
    class AudioFileStream(object):
        ...
        def read(self, size=-1,  split=False):
            ...
            if self.audio_reader.getnchannels() != 1 and not split:  # stereo audio
                buffer = audioop.tomono(buffer, sample_width, 1, 1)  # convert stereo audio data to mono
            elif self.audio_reader.getnchannels() != 1 and split:
                buffer = [audioop.tomono(buffer, sample_width, 1, 0), audioop.tomono(buffer, sample_width, 0, 1)]
            return buffer
...
class Recognizer(AudioSource):
    ...
    def record(self, source, duration=None, offset=None, stereo_to_mono=False):
        """
        ...
        while True:  # loop for the total number of chunks needed
            if offset and not offset_reached:
                ...
                if offset_time > offset:
                    offset_reached = True
            buffer = source.stream.read(source.CHUNK, split=split)
            if split:
                # In this case buffer will be a list of left, right audio
                if len(buffer[0]) == 0 or len(buffer[1]) == 0: break
            else:
            ...
            if offset_reached or not offset:
                ...
                if split:
                    frames[0].write(buffer[0])
                    frames[1].write(buffer[1])
                else:
                    frames.write(buffer)
        if split:
            frame_data = []
            ...
            for i, f in enumerate(frames):
                frame_data.append(f.getvalue())
                f.close()
                audio_data.append(AudioData(frame_data[i], source.SAMPLE_RATE, source.SAMPLE_WIDTH))
    ...
    def write_flac(self, audio_data, filename):
    ...
    def recognize_google_cloud(self, audio_data, credentials_json=None, language="en-US", preferred_phrases=None, show_all=False):
        ...
        request = speech_service.speech().asyncrecognize(body={"audio": {"content": base64.b64encode(flac_data).decode("utf8")}, "config": speech_config})
